...
const testCases = [
  ...
  {
    name: 'Small-Cap Fund Comparison',
    data: {
      fundCodes: ['SBISMALLCAP'],
      category: 'Small Cap',
      period: '1y',
...
async function testFundComparison(testCase) {
  try {
    ...
    const response = await axios.post(`${BASE_URL}/api/analytics/compare-funds`, testCase.data, {
      headers: {
    ...
    if (response.status === 200 && response.data.success) {
      ...
      const validation = validateResponse(data, testCase);
      
      if (validation.isValid) {
        console.log(`✅ PASSED: ${testCase.name}`);
        console.log(`📈 Top performer: ${data.summary.topPerformer.fundName} (${data.summary.topPerformer.rating}⭐)`);
        console.log(`💰 Best value: ${data.summary.bestValue.fundName}`);
        console.log(`🛡️  Safest choice: ${data.summary.safestChoice?.fundName || 'N/A'}`);
        
        results.passed++;
        results.details.push({
          test: testCase.name,
          status: 'PASSED',
          topPerformer: data.summary.topPerformer.fundName,
          bestValue: data.summary.bestValue.fundName
        });
      } else {
      ...
      console.log(`❌ FAILED: ${testCase.name} - Invalid response`);
...
async function testErrorCases() {
  ...
  const errorTests = [
    ...
    {
      name: 'Invalid fund codes (not array)',
      data: { fundCodes: 'HDFCMIDCAP' },
      expectedError: 'Please provide 2-5 fund codes for comparison'
    }
  ];
  ...
  for (const test of errorTests) {
    try {
      const response = await axios.post(`${BASE_URL}/api/analytics/compare-funds`, test.data, {
        headers: {
          'Authorization': `Bearer ${TEST_TOKEN}`,
          'Content-Type': 'application/json'
        }
      ...
      if (response.status === 400 && response.data.message === test.expectedError) {
      ...
      if (error.response && error.response.status === 400 && 
          error.response.data.message === test.expectedError) {
        console.log(`✅ PASSED: ${test.name}`);
        results.passed++;
      } else {
...
async function testPerformance() {
  ...
  try {
    const response = await axios.post(`${BASE_URL}/api/analytics/compare-funds`, {
      fundCodes: ['HDFCMIDCAP', 'ICICIBLUECHIP', 'SBISMALLCAP', 'AXISBLUECHIP', 'MIRAEEMERGING'],
      period: '1y',
      investmentAmount: 100000,
      includeRatings: true,
    ...
    if (response.status === 200 && duration < 10000) { // Should complete within 10 seconds
...
async function runTests() {
  console.log('🚀 Starting Fund Comparison API Tests...\n');
  ...
  if (results.failed === 0) {
    console.log('\n🎉 All tests passed! Fund comparison API is working correctly.');
  } else {
